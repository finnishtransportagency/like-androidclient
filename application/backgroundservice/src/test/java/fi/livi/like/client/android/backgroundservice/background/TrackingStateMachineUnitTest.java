...
import fi.livi.like.client.android.backgroundservice.tracking.Journey;
...
public class TrackingStateMachineUnitTest {
    ...
    private LocationBearingAngleTracker bearingAngleTracker;
    ...
    public void setup() {
        ...
        TrackingDisabledHandler trackingDisabledHandler = mock(TrackingDisabledHandler.class);
        broadcaster = mock(Broadcaster.class);
        distanceCalculator = mock(DistanceCalculator.class);
        bearingAngleTracker = mock(LocationBearingAngleTracker.class);
        Configuration configuration = mock(Configuration.class);

        trackingStateMachine = new TrackingStateMachine(
                stateMachineClient, trackingDisabledHandler, broadcaster,
                distanceCalculator, bearingAngleTracker, configuration);
    ...
    public void should_change_state_from_user_moving_with_previous_journey_to_tracking_user_when_receiving_location_far_enough() {
        ...
        when(distanceCalculator.distanceBetween(anyDouble(), anyDouble(), anyDouble(), anyDouble())).thenReturn(10.01f);
        when(mockedJourney.getLastLikeLocation()).thenReturn(mockedLikeLocation);
        when(bearingAngleTracker.isBearingAngleWithinLimits(mockedLocation)).thenReturn(true);
    ...
    @Test
    public void should_not_change_state_from_user_moving_with_previous_journey_to_tracking_user_when_receiving_location_far_enough_but_bearing_not_within_limits() {
        trackingStateMachine.setTrackingState(TrackingStateMachine.State.WAITING_MOVEMENT);
        Journey mockedJourney = mock(Journey.class);
        Location mockedLocation = mock(Location.class);
        when(mockedLocation.getSpeed()).thenReturn(0.001f);
        ...
        when(distanceCalculator.distanceBetween(anyDouble(), anyDouble(), anyDouble(), anyDouble())).thenReturn(10.01f);
        when(mockedJourney.getLastLikeLocation()).thenReturn(mockedLikeLocation);
        when(bearingAngleTracker.isBearingAngleWithinLimits(mockedLocation)).thenReturn(false);
    ...
    @Test
    public void should_not_change_state_from_waiting_with_previous_journey_to_tracking_user_when_receiving_location_with_null_journey_when_bearing_not_within_limits() {
        trackingStateMachine.setTrackingState(TrackingStateMachine.State.WAITING_MOVEMENT);
        Location mockedLocation = mock(Location.class);
        when(mockedLocation.getSpeed()).thenReturn(0.1f);
        Journey mockedJourney = mock(Journey.class);
